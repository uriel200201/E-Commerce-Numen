...
const Product = ({ index, product, addToCart }) => {
  ...
  return (
      <li className="product bg-white w-[200px] space-y-3 shadow-lg p-2 mx-5 rounded-[5px] opacity-0 pointer-events-none scale-75 transition-all duration-300">
        ...
        <p>US$ {price}</p>
        <button disabled={loading ? true : false} className='w-full h-[40px] rounded-[3px] text-red-500 hover:bg-red-400 hover:text-white disabled:pointer-events-none' onClick={() => setOpen(true)}>{loading ? <Loader /> : <span>Agregar</span>}</button>
        <ModalProduct
          open={open}
          setOpen={setOpen}
          info={product}
